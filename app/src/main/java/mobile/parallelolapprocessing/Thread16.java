 ...
 */
public class Thread16 extends AsyncTask {
    ...
    public Thread16(List<List<List<Integer>>> allAxisDetails, List<Integer> selectedMeasures, HashMap<Integer, String> measureMap,
    ...
    public Thread16()
    {
        if(Thread16.inflatedQueries == null)
        {
            Thread16.inflatedQueries = new ArrayList<>();
        }
    ...
    protected Object doInBackground(Object[] params) {
        Log.d("Inflated Query 15:", "Time taken to start job " + String.valueOf(System.currentTimeMillis()));
        this.run();
        long end = System.currentTimeMillis();
        Log.d("Inflated Query 15:", "Time taken to finish job " + String.valueOf(end));
    ...
    public void run() {
        try {
            ...
            if(allLeaves.size()>0) {
                ...
                if(!Thread16.inflatedQueries.contains(queryListForSiblings.get(0)) ) {
                    Thread16.inflatedQueries.add(queryListForSiblings.get(0));
                    String query =  "select {[Measures].[Internet Tax Amount]} on axis(0), DESCENDANTS({[Geography].[Geography].[All Geographies].[France]},2,LEAVES) on axis(1) ,DESCENDANTS({[Employee].[Employee Department].[All Employees]},1,LEAVES) on axis(2) from [Adventure Works]";
