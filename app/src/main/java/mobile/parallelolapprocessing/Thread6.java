 ...
 */
public class Thread6 extends AsyncTask{
    ...
    public Thread6(List<List<List<Integer>>> allAxisDetails, List<Integer> selectedMeasures, HashMap<Integer, String> measureMap,
    ...
    public Thread6()
    {
        if(Thread6.inflatedQueries == null)
        {
            Thread6.inflatedQueries = new ArrayList<>();
        }
    ...
    protected Object doInBackground(Object[] params) {
        Log.d("Inflated Query 5:", "Time taken to start job " + String.valueOf(System.currentTimeMillis()));
        this.run();
        long end = System.currentTimeMillis();
        Log.d("Inflated Query 5:", "Time taken to finish job " + String.valueOf(end));
    ...
    public void run() {
        try {
            ...
            if(allLeaves.size()>0) {
                ...
                if(!Thread6.inflatedQueries.contains(queryListForSiblings.get(0))) {
                    Thread6.inflatedQueries.add(queryListForSiblings.get(0));
                    String query = "select {[Measures].[Internet Sales Amount]} on axis(0), DESCENDANTS({[Geography].[Geography].[All Geographies]},2,LEAVES) on axis(1) ,DESCENDANTS({[Employee].[Employee Department].[All Employees]},1,LEAVES) on axis(2) from [Adventure Works]";

                    //"select {[Measures].[Internet Sales Amount]} on axis(0), DESCENDANTS({[Customer].[Education].[All Customers].[Bachelors],[Customer].[Education].[All Customers].[Graduate Degree],[Customer].[Education].[All Customers].[Partial College],[Customer].[Education].[All Customers].[Partial High School]},0,LEAVES) on axis(1) ,DESCENDANTS({[Date].[Calendar].[All Periods].[CY 2007]},1,LEAVES) on axis(2)  from [Adventure Works]";

                    //"select {[Measures].[Internet Sales Amount]} on axis(0), DESCENDANTS({[Customer].[Education].[All Customers].[Graduate Degree],[Customer].[Education].[All Customers].[High School],[Customer].[Education].[All Customers].[Partial College],[Customer].[Education].[All Customers].[Partial High School]},0,LEAVES) on axis(1) ,DESCENDANTS({[Date].[Calendar].[All Periods].[CY 2007]},4,LEAVES) on axis(2) ,DESCENDANTS({[Geography].[Geography].[All Geographies]},4,LEAVES) on axis(3) from [Adventure Works]";//String query// = "select {[Measures].[Internet Sales Amount]} on axis(0), DESCENDANTS({[Employee].[Employee Department].[All Employees].[Document Control],[Employee].[Employee Department].[All Employees].[Engineering],[Employee].[Employee Department].[All Employees].[Executive],[Employee].[Employee Department].[All Employees].[Facilities and Maintenance],[Employee].[Employee Department].[All Employees].[Finance],[Employee].[Employee Department].[All Employees].[Human Resources],[Employee].[Employee Department].[All Employees].[Information Services],[Employee].[Employee Department].[All Employees].[Marketing]},2,LEAVES) on axis(1) ,DESCENDANTS({[Geography].[Geography].[All Geographies].[Australia],[Geography].[Geography].[All Geographies].[Canada],[Geography].[Geography].[All Geographies].[France],[Geography].[Geography].[All Geographies].[United Kingdom],[Geography].[Geography].[All Geographies].[United States]},3,LEAVES) on axis(2) from [Adventure Works]";
