...
/**
 * Created by jayma on 4/8/2016.
 */
public class Thread12 extends AsyncTask {
    ...
    public Thread12(List<List<List<Integer>>> allAxisDetails, List<Integer> selectedMeasures, HashMap<Integer, String> measureMap,
    ...
    public Thread12()
    {
        if(Thread12.inflatedQueries == null)
        {
            Thread12.inflatedQueries = new ArrayList<>();
        }
    ...
    protected Object doInBackground(Object[] params) {
        Log.d("Inflated Query 11:", "Time taken to start job " + String.valueOf(System.currentTimeMillis()));
        this.run();
        long end = System.currentTimeMillis();
        Log.d("Inflated Query 11:", "Time taken to finish job " + String.valueOf(end));
    ...
    public void run() {
        try {
            ...
            if(allLeaves.size()>0) {
                ...
                if(!Thread12.inflatedQueries.contains(queryListForSiblings.get(0)) ) {
                    Thread12.inflatedQueries.add(queryListForSiblings.get(0));
                    String query =  "select {[Measures].[Internet Sales Amount]} on axis(0), DESCENDANTS({[Geography].[Geography].[All Geographies].[Australia]},1,LEAVES) on axis(1) ,DESCENDANTS({[Employee].[Employee Department].[All Employees]},2,LEAVES) on axis(2) from [Adventure Works]";
