 ...
 */
public class Thread14 extends AsyncTask {
    ...
    public Thread14(List<List<List<Integer>>> allAxisDetails, List<Integer> selectedMeasures, HashMap<Integer, String> measureMap,
    ...
    public Thread14()
    {
        if(Thread14.inflatedQueries == null)
        {
            Thread14.inflatedQueries = new ArrayList<>();
        }
    ...
    protected Object doInBackground(Object[] params) {
        Log.d("Inflated Query 13:", "Time taken to start job " + String.valueOf(System.currentTimeMillis()));
        this.run();
        long end = System.currentTimeMillis();
        Log.d("Inflated Query 13:", "Time taken to finish job " + String.valueOf(end));
    ...
    public void run() {
        try {
            ...
            if(allLeaves.size()>0) {
                ...
                if(!Thread14.inflatedQueries.contains(queryListForSiblings.get(0)) ) {
                    Thread14.inflatedQueries.add(queryListForSiblings.get(0));
                    String query =  "select {[Measures].[Internet Tax Amount]} on axis(0), DESCENDANTS({[Geography].[Geography].[All Geographies].[Australia]},2,LEAVES) on axis(1) ,DESCENDANTS({[Employee].[Employee Department].[All Employees]},1,LEAVES) on axis(2) from [Adventure Works]";
