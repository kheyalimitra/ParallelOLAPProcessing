        ...
        try {
            ...
            if(allLeaves.size()>0) {
                ...
                if(!Thhread4.inflatedQueries.contains(queryListForSiblings.get(0)) && !CacheProcess.inflatedQueries.contains(queryListForSiblings.get(0))) {
                    ...
                    String query = "select {[Measures].[Internet Sales Amount]} on axis(0), DESCENDANTS({[Product].[Product Categories].[All Products]},3,LEAVES) on axis(1) ,DESCENDANTS({[Geography].[Geography].[All Geographies].[Australia],[Geography].[Geography].[All Geographies].[Canada],[Geography].[Geography].[All Geographies].[France]},3,LEAVES) on axis(2) from [Adventure Works]";
