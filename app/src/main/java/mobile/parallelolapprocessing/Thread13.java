 ...
 */
public class Thread13  extends AsyncTask {
    ...
    public Thread13(List<List<List<Integer>>> allAxisDetails, List<Integer> selectedMeasures, HashMap<Integer, String> measureMap,
    ...
    public Thread13()
    {
        if(Thread13.inflatedQueries == null)
        {
            Thread13.inflatedQueries = new ArrayList<>();
        }
    ...
    protected Object doInBackground(Object[] params) {
        Log.d("Inflated Query 12:", "Time taken to start job " + String.valueOf(System.currentTimeMillis()));
        this.run();
        long end = System.currentTimeMillis();
        Log.d("Inflated Query 12:", "Time taken to finish job " + String.valueOf(end));
    ...
    public void run() {
        try {
            ...
            if(allLeaves.size()>0) {
                ...
                if(!Thread13.inflatedQueries.contains(queryListForSiblings.get(0)) ) {
                    Thread13.inflatedQueries.add(queryListForSiblings.get(0));
                    String query =  "select {[Measures].[Internet Sales Amount]} on axis(0), DESCENDANTS({[Geography].[Geography].[All Geographies].[Australia]},2,LEAVES) on axis(1) ,DESCENDANTS({[Employee].[Employee Department].[All Employees]},1,LEAVES) on axis(2) from [Adventure Works]";
