 ...
 */
public class thread11 extends AsyncTask {
    ...
    public thread11(List<List<List<Integer>>> allAxisDetails, List<Integer> selectedMeasures, HashMap<Integer, String> measureMap,
    ...
    public thread11()
    {
        if(thread11.inflatedQueries == null)
        {
            thread11.inflatedQueries = new ArrayList<>();
        }
    ...
    protected Object doInBackground(Object[] params) {
        Log.d("Inflated Query 10:", "Time taken to start job " + String.valueOf(System.currentTimeMillis()));
        this.run();
        long end = System.currentTimeMillis();
        Log.d("Inflated Query 10:", "Time taken to finish job " + String.valueOf(end));
    ...
    public void run() {
        try {
            ...
            if(allLeaves.size()>0) {
                ...
                if(!thread11.inflatedQueries.contains(queryListForSiblings.get(0)) ) {
                    thread11.inflatedQueries.add(queryListForSiblings.get(0));
                    String query =  "select {[Measures].[Internet Sales Amount]} on axis(0), DESCENDANTS({[Geography].[Geography].[All Geographies].[Canada].[British Columbia]},1,LEAVES) on axis(1) ,DESCENDANTS({[Employee].[Employee Department].[All Employees]},2,LEAVES) on axis(2) from [Adventure Works]";
