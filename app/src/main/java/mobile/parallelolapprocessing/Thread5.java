 ...
 */
public class Thread5 extends AsyncTask{
    ...
    public Thread5(List<List<List<Integer>>> allAxisDetails, List<Integer> selectedMeasures, HashMap<Integer, String> measureMap,
    ...
    public Thread5()
    {
        if(Thread5.inflatedQueries == null)
        {
            Thread5.inflatedQueries = new ArrayList<>();
        }
    ...
    protected Object doInBackground(Object[] params) {
        Log.d("Inflated Query 4:", "Time taken to start job " + String.valueOf(System.currentTimeMillis()));
        this.run();
        long end = System.currentTimeMillis();
        Log.d("Inflated Query 4:", "Time taken to finish job " + String.valueOf(end));
    ...
    public void run() {
        try {
            ...
            if(allLeaves.size()>0) {
                ...
                if(!Thread5.inflatedQueries.contains(queryListForSiblings.get(0))) {
                    Thread5.inflatedQueries.add(queryListForSiblings.get(0));
                    String query = "select {[Measures].[Internet Sales Amount]} on axis(0), DESCENDANTS({[Geography].[Geography].[All Geographies].[United States]},2,LEAVES) on axis(1) ,DESCENDANTS({[Date].[Calendar].[All Periods].[CY 2008]},1,LEAVES) on axis(2) from [Adventure Works]";
