...
/**
 * Created by jayma on 12/05/2016.
 */
public class Thread17 extends AsyncTask {
    ...
    public Thread17(List<List<List<Integer>>> allAxisDetails, List<Integer> selectedMeasures, HashMap<Integer, String> measureMap,
    ...
    public Thread17()
    {
        if(Thread17.inflatedQueries == null)
        {
            Thread17.inflatedQueries = new ArrayList<>();
        }
    ...
    protected Object doInBackground(Object[] params) {
        Log.d("Inflated Query 16:", "Time taken to start job " + String.valueOf(System.currentTimeMillis()));
        this.run();
        long end = System.currentTimeMillis();
        Log.d("Inflated Query 16:", "Time taken to finish job " + String.valueOf(end));
        return "Success";
    }
    ...
    public void run() {
        try {
            ...
            if(allLeaves.size()>0) {
                ...
                if(!Thread17.inflatedQueries.contains(queryListForSiblings.get(0)) ) {
                    Thread17.inflatedQueries.add(queryListForSiblings.get(0));
                    String query =  "select {[Measures].[Internet Tax Amount]} on axis(0), DESCENDANTS({[Geography].[Geography].[All Geographies].[Germany]},2,LEAVES) on axis(1) ,DESCENDANTS({[Employee].[Employee Department].[All Employees]},1,LEAVES) on axis(2) from [Adventure Works]";
